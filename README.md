...
这是一个全栈业务项目脚手架，包含 Spring Boot 后端、Vue Web 前端、原生微信小程序、FastAPI AI 助手服务、MySQL 脚本和 Figma-first 开发工作流。
...
- 第 2 步默认生成可复制的 Lovart 单页提示词；如已安装 `lovart-skill` 并配置 `LOVART_ACCESS_KEY` / `LOVART_SECRET_KEY`，可自动生成单页原型图到 `design/lovart/pages/`，但不能因此扩大页面范围。自动出图固定优先 `generate_image_gpt_image_2_medium`，APP/小程序默认 9:16，网页默认 16:9，每个新业务需求都新建以业务中文名命名的 Lovart Project。
