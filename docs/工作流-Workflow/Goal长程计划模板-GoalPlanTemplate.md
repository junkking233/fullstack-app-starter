...
状态只能使用：待开始、进行中、待验收、已完成、阻塞、不适用。第 4 步的任务必须按模块继续拆细到页面、接口、数据、状态、资源和联调检查，不允许只保留“Web 实现”“小程序实现”这种无法验收的大项。
...
| 4 | API 子任务 | `docs/技术设计-TechDesign/API接口文档-API.md` | 阶段 1-3 | 契约完整 | 待开始 |
| 4 | DB 子任务 | `db/db.sql` | API 子任务 | 数据支撑功能 | 待开始 |
| 4 | Goal 执行计划 | `docs/工作流-Workflow/Goal长程计划-GoalPlan.md`、`index.html` | 阶段 1-3 | 可恢复执行 | 待开始 |
| 4 | 后端实现 | `apps/backend/` | API / DB / Goal | 接口一致 | 待开始 |
| 4 | Web 实现 | `apps/frontend/` | DESIGN / UI Design / Goal | 逐页还原设计稿并记录证据 | 待开始 |
| 4 | 小程序实现 | `apps/miniprogram/` | DESIGN / UI Design / Goal | 逐页还原设计稿并记录证据 | 待开始 |
| 4 | AI 助手实现 | `apps/aiassistant/` | API / DB / Goal | 接口可用或明确不适用 | 待开始 |
| 5 | 对抗式审查、修复与提交 | 全项目 | 第 4 步完成门禁 | 问题修复并提交 | 待开始 |

## 8. Goal 完成门禁

第 4 步完成前必须逐项检查本节。没有全部通过时，当前阶段仍是“基于 Figma 设计稿实现功能”，不得推进到“对抗式审查、修复与提交”。
...
- 只有所有必做列为已完成或不适用，且模块状态为已通过，第 4 步才能完成。
- 存在待完成、待验收、阻塞或空白时，不得进入第 5 步；下一步必须写具体剩余任务。
