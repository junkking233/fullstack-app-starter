...
| 4 | 基于 Figma 设计稿实现功能 | `apps/`、`db/db.sql`、`docs/技术设计-TechDesign/API接口文档-API.md`、`docs/工作流-Workflow/Goal长程计划-GoalPlan.md`（按需） | 5 |
| 5 | 对抗式审查、修复与提交 | 问题清单、修复提交 | 完成 |
...
- 阶段证据只认正式产物，不认模板文件。例如阶段 2 看 `LovartPrompt.md`，阶段 3 看 `DESIGN.md` 和 `UIDesign.md`，阶段 4 看 `API.md`、`db/db.sql`、`GoalPlan.md`（如存在）和代码实现。
...
- 第 4 步进入第 5 步前，必须通过“Goal 完成门禁”：GoalPlan 中所有必做模块、页面、接口、数据、状态和资源项都已完成或明确标为不适用。
...
- API、数据库和 Goal 计划是本步骤的实现前置子任务，不再拆成独立阶段。根据 PRD 和 UI Design 判断必须实现的页面、接口、数据和暂不开发范围，分别更新 `docs/技术设计-TechDesign/API接口文档-API.md`、`db/db.sql` 和 GoalPlan。
- API 文档必须建立“角色 x 模块 x 操作”矩阵；每个接口写清路径、方法、权限、参数、返回、错误码和业务说明；统一响应结构为 `code`、`message`、`data`、`timestamp`。
- 数据库根据 API 抽取实体、关系、状态、索引和种子数据；只更新 `db/db.sql`，不创建 MySQL 容器，不重置已有数据库。
- 功能较大、整套系统开发或用户明确要求长程任务时，基于 `docs/工作流-Workflow/Goal长程计划模板-GoalPlanTemplate.md`（如存在）生成或更新 GoalPlan，覆盖需求、设计稿、API、数据库、实现拆解、验收、风险和执行记录。
- 后端严格按 API 文档实现。
...
- 每完成模块，更新状态文件和 Goal 执行记录，但不自动推进到第 5 步。
- 进入第 5 步前必须逐项核对 GoalPlan 的实现拆解和验收矩阵；有任何必做项仍为待开始、进行中、待验收、待完成、阻塞或空白，就继续停留在第 4 步。
- 无法运行服务或测试时，可以记录静态检查结果和阻塞原因，但不能把未验证的必做项标为完成。

### 5. 对抗式审查、修复与提交

- 审查开始前先确认第 4 步已通过 Goal 完成门禁；如果未通过，退回第 4 步补齐实现。
- 主动查需求遗漏、设计稿还原不足、接口漂移、数据库缺口、权限问题、交互缺口和测试盲区。
- 对抗式审查只做最终兜底；如果第 4 步缺少逐页 UI 还原记录，应退回功能实现，不在审查阶段重新补整套页面。
- 输出问题、证据、影响、严重程度、责任 skill 和修复建议。
- 确认的问题直接修复，修复后回归检查；不自动启动服务，服务未启动时只做静态验收并标记阻塞。
- 修复只改相关文件，不做无关重构。没有问题后提交代码。

## 常用入口

| 任务 | 做法 |
| --- | --- |
| 整套系统 | 0-4 完整实现，再审查修复提交 |
| 单功能 | 先读状态，补缺失阶段，再实现 |
| 只做后端 | 确认 API 和数据库足够 |
