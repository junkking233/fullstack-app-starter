...
6. 当前任务相关文档，例如 `docs/需求文档.md`、`docs/原型生图提示词.md`、`docs/页面设计.md`、`docs/API文档.md`、`db/db.sql`
...
| 3 | Lovart 设计稿拆解 | 读取 Lovart 原型图、PSD、PNG，拆解页面结构、状态和资源 | `docs/页面设计.md`、`design/lovart/` | 主 Agent / frontend-worker |
| 4 | API 设计 | 定义前后端与小程序统一接口契约 | `docs/API文档.md` | api-designer |
| 5 | 数据库设计 | 设计表结构、状态字段、索引、种子数据 | `db/db.sql` | db-builder |
| 6 | Goal 计划定稿 | 把需求、原型、页面、接口、数据库和实现拆解合并 | `docs/GOAL_PLAN.md` | 主 Agent |
| 7 | 功能实现 | 按计划实现后端、Web、小程序、可选 AI 助手服务 | `apps/`、`db/` | backend-worker / frontend-worker |
| 8 | 对抗式代码审查 | 主动找需求遗漏、契约漂移、权限与测试缺口 | 审查问题清单 | regression-tester / 主 Agent |
| 9 | 回归验收与修复 | 静态验收、必要运行验收、修复和复测 | 验收结果、修复提交 | regression-tester / 对应 worker |
...
- 基于 `docs/需求文档.md` 生成 `docs/原型生图提示词.md`，供用户复制到 Lovart 生成 APP 原型图、交互概览图和 PSD/PNG 设计稿。
- 原型生图提示词必须包含整套 APP UI 总览提示词、4 个核心页面单独提示词、4 个页面交互概览图提示词。
- 每个页面交互概览图必须体现 1 个主界面和 3-5 个相关操作状态，例如弹窗、底部弹层、筛选面板、详情页、编辑态、成功态。
- 原型提示词只用于设计生成，不进入接口、数据库或代码实现。
- 完成后更新 `docs/WORKFLOW_STATE.md`，下一步指向阶段 3，并等待用户把 Lovart 生成的原型图、PSD、PNG 放入项目。

### 5.3 Lovart 设计稿拆解

- 默认从 `design/lovart/` 读取 Lovart 生成的原型图片、PSD、PNG；如果用户提供了其他目录，以实际路径为准。
- 同时读取 `docs/原型生图提示词.md` 和设计稿文件，确保页面拆解与原型提示词一致。
- 输出页面清单：端、页面、角色、入口、主要功能、接口依赖、状态反馈。
- 准备 `docs/页面设计.md`，覆盖设计稿来源、目标页面路径、布局、组件、表单字段、列表字段、空状态、加载状态、失败反馈、权限不可见状态和交互状态。
- PSD/PNG 是页面还原依据；实现前不要擅自重设计页面风格、颜色、间距或组件层级。
- 图标必须先检查 `assets/icons/`；缺少图标先补共享图标源库，再复制到对应端。
- 如需要，可使用 `npx @google/design.md` 生成补充页面设计文档，但不得替代 Lovart 设计稿。
- 完成后更新 `docs/WORKFLOW_STATE.md`，下一步指向阶段 4。
...
- Goal 计划必须包含目标、输入资料、需求分析、原型生图提示词、Lovart 设计稿拆解、页面清单、API 计划、数据库计划、实现拆解、验收矩阵、对抗式审查清单、风险阻塞、执行记录。
- 不要简化成普通 TODO。
- 完成后更新 `docs/WORKFLOW_STATE.md`，下一步指向阶段 7。
...
- 前端和小程序页面必须以 `docs/页面设计.md`、Lovart 原型图、PSD、PNG 为视觉还原依据。
...
- 主动检查需求遗漏、原型提示词遗漏、Lovart 设计稿还原不足、接口契约漂移、数据库支撑不足、权限问题、页面交互缺口、测试盲区。
...
| 任务类型 | 执行方式 |
| --- | --- |
| 一次开发整个系统 | 先走阶段 0-6，再进入阶段 7 实现，最后阶段 8-9 审查验收。 |
| 开发单个功能模块 | 先读状态文件，缺需求/接口/数据库就补对应阶段，再实现和验收。 |
| 只开发后端接口 | 确认 API 和数据库足够支撑，只改后端和必要文档，完成后更新状态。 |
| 只开发 Web 前端或小程序 | 确认 `docs/页面设计.md` 和接口文档足够支撑页面；Web 用 Element Plus，小程序用原生微信 + Vant Weapp。 |
