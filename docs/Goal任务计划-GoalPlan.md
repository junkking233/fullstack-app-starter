...
- [ ] 3. Stitch 重建与 Figma 设计稿拆解：如使用 Stitch，已重建 UI screen 并导出/同步到 Figma 或 HTML；生成 `docs/设计还原文档-UIDesign.md`。
...
- [ ] Stitch 输出：screenId、HTML、截图或导出记录已保存到 `design/stitch/` 或 `.stitch/`；无法执行时已写明原因。
...
- [ ] 阶段 3 如使用 Stitch，已记录 Stitch Project、screenId、来源 Lovart 图、页面提示词、导出/同步到 Figma 或 HTML 的结果；没有 Figma Frame 时不得进入第 4 步完成态。
