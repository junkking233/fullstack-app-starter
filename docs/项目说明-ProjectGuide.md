...
| 3. Stitch 重建与 Figma 设计稿拆解 | 用 Lovart 图片和页面提示词在 Stitch 重建 UI screen，再从 Stitch 页面 Copy/Paste 到目标 Figma 文件，最后拆解 Figma Frame | `design/stitch/`、`设计还原文档-UIDesign.md` |
...
- Lovart 用来生成单页原型图，出图后通过 Stitch Copy/Paste 或手动整理形成真实 Figma Frame。
...
- 推荐设计链路是 `Lovart 单页图 -> Stitch UI screen -> 在 Stitch 页面 Copy/Paste 到 Figma -> Figma Frame -> 代码实现`。
...
- Stitch 生成后，在 Stitch 页面使用 Copy/Paste to Figma，把 screen 放入目标 Figma 文件，并按 `Pxx-页面名称` 命名 Frame。
