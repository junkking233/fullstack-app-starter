...
- 任何开发任务开始前，必须先读取 `docs/DEVELOPMENT_WORKFLOW.md` 和 `docs/WORKFLOW_STATE.md`，确认当前阶段、最近完成、下一步和阻塞项。
...
- 开发顺序固定为：产品需求分析（第一性原理） -> 原型生图提示词生成 -> Lovart 设计稿拆解 -> API 设计 -> 数据库设计 -> Goal 计划定稿 -> 实现 -> 对抗式代码审查 -> 回归验收。
- 需求分析阶段要明确用户角色、业务对象、核心流程、系统边界和待确认问题；不确定内容必须标注为“待确认”。
- 原型生图提示词阶段要基于 `docs/需求文档.md` 生成 `docs/原型生图提示词.md`，供用户复制到 Lovart 生成 APP 原型图、交互概览图和 PSD/PNG 设计稿。
- Lovart 设计稿拆解阶段要读取 `docs/原型生图提示词.md` 和用户放入项目的原型图、PSD、PNG，生成或更新 `docs/页面设计.md`；设计稿默认放在 `design/lovart/`。
- 页面实现必须以 Lovart 原型图、PSD、PNG 和 `docs/页面设计.md` 为视觉还原依据，不能跳过设计稿直接自由发挥。
- 实现阶段必须按 `docs/GOAL_PLAN.md` 推进，每完成一个阶段更新计划状态和发现的问题。
- 对抗式代码审查阶段要主动检查需求遗漏、设计稿还原不足、接口契约不一致、数据库支撑不足、权限问题、页面交互缺口和测试盲区。
- 每完成一个阶段或局部任务，必须更新 `docs/WORKFLOW_STATE.md`，写清当前阶段、最近完成、下一步、阻塞项和证据路径。
- 如果没有上下文，优先根据 `docs/WORKFLOW_STATE.md` 判断该继续需求分析、设计、实现、审查还是验收，不要直接猜测下一步。
...
- 子应用目录可以保留各自的 `README.md` 作为开发入口；不要额外生成总结性文档。
